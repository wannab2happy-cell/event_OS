...
export async function updateParticipantNotesAction(input: unknown) {
  try {
    ...
    console.error('updateParticipantNotesAction error:', error);
    return {
      success: false,
      message: error?.message || '메모 업데이트 중 오류가 발생했습니다.',
    };
  }
}
