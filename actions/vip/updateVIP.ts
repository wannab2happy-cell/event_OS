...
// 간단한 유효성 검사
function validateInput(input: unknown): UpdateVIPInput {
  ...
  if (typeof data.vipLevel !== 'number' || data.vipLevel < 0 || data.vipLevel > 3) {
    throw new Error('vipLevel must be between 0 and 3');
  }
...
/**
 * VIP 정보 수정 Server Action
 */
export async function updateVIPAction(input: unknown): Promise<{ success: boolean; message?: string }> {
  try {
    ...
    if (updateError) {
      throw new Error(`VIP 정보 수정에 실패했습니다: ${updateError.message}`);
    }
    ...
    return {
      success: true,
      message: 'VIP 정보가 성공적으로 수정되었습니다.',
    };
  } catch (error: any) {
    console.error('updateVIPAction error:', error);
    return {
      success: false,
      message: error?.message || 'VIP 정보 수정 중 오류가 발생했습니다.',
    };
  }
}
